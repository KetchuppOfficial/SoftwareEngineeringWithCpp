...
class RationalWithMixins final
  ...
  public:
    ...
    constexpr friend bool operator==(const RationalWithMixins &lhs, const RationalWithMixins &rhs) {
        return lhs.num_ == rhs.num_ && lhs.den_ == rhs.den_;
    }

    constexpr friend std::strong_ordering operator<=>(const RationalWithMixins &lhs,
                                                      const RationalWithMixins &rhs) {
        return lhs.num_ * rhs.den_ <=> rhs.num_ * lhs.den_;
    }

    friend std::ostream &operator<<(std::ostream &os, const RationalWithMixins &rational) {
        return os << rational.num_ << '/' << rational.den_;
    }

    friend std::istream &operator>>(std::istream &is, RationalWithMixins &rational) {
        is >> rational.num_;
