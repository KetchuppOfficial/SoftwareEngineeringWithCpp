...
constexpr std::string_view kTarget = "methinksitislikeaweasel";
constexpr std::size_t kStringsCount = 100;
constexpr unsigned kMutationProb = 5;

std::size_t count_equal_chars(std::string_view first, std::string_view second) {
...
int main() try {
    ...
    for (;;) {
        ...
        auto scores = strings | std::views::transform([](std::string_view sv) static {
                          return count_equal_chars(sv, kTarget);
                      });
