...
TEST(MAE, EmptyRange) {
    // Act
    const auto res = dts::ranges::mae(std::views::empty<int>);

    // Assert
    ASSERT_FALSE(res.has_value());
}

TEST(MAE, NonEmptyForwardRange) {
    // Assign
    ...
    // Act
    const auto res = dts::ranges::mae(errors);

    // Assert
    ASSERT_TRUE(res.has_value());
    EXPECT_EQ(*res, 13.0 / 9.0);
}

TEST(MAE, NonEmptyRandomAccessRange) {
    // Assign
