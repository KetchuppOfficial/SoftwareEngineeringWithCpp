...
class SortBenchmark : public benchmark::Fixture {
  public:
    SortBenchmark() {
        ...
        if (!env_var) {
            throw std::runtime_error{std::format("SORT_THRESHOLD environment variable is not set")};
        }

        const std::string_view env_sv{env_var};
        const auto [ptr, ec] =
            std::from_chars(env_sv.data(), env_sv.data() + env_sv.size(), threshold_);
        if (ec == std::errc::invalid_argument) {
            ...
            throw std::runtime_error{std::format("unparsed characters in string \'{}\'", env_sv)};
        }
    }
