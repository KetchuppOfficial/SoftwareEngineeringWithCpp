...
int main() try {
    ...
    if (const auto maybe_roots = solve(a, b, c); !maybe_roots.has_value()) {
        ...
        struct Visitor final {
            static void operator()([[maybe_unused]] InfRoots) {
                std::println("infinite number of roots");
            }

            static void operator()(OneRoot root) { std::println("one root: {:.2f}", root); }

            static void operator()(TwoRoots roots) {
                std::println("two roots: ({:.2f}, {:.2f})", roots.first, roots.second);
            }
        };

        std::visit(Visitor{}, *maybe_roots);
    }
